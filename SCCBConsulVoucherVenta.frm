...
Begin VB.Form frmCBConsulVoucherVenta 
   Caption         =   "Consulta de Voucher de Ventas"
   ClientHeight    =   8595
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11880
   HelpContextID   =   110
   Icon            =   "SCCBConsulVoucherVenta.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8595
   ScaleWidth      =   11880
   ...
   End
End
Attribute VB_Name = "frmCBConsulVoucherVenta"
...
Dim mcurAsientosVentas As New clsBD2
Dim mcurVentas As New clsBD2
Dim mcolVentasProdServ As New Collection
...
' Nombre del fichero
  rptCBVoucher.NombreRPT = "RPTCBASIENTOVOUCHERVentas.rpt"
...
'Sentencia SQL
 sSQL = ""
 sSQL = "DELETE * FROM RPTCBVOUCHERVENTAS"
  
'Borra la tabla
 Var21 sSQL
 
 sSQL = ""
 sSQL = "DELETE * FROM RPTCBASIENTOVOUCHERVENTAS"
...
For i = 1 To grdConsulta.Rows - 1
    If Trim(grdConsulta.TextMatrix(i, 0)) <> Empty Then
        ...
        If grdConsulta.TextMatrix(i, 0) = "M" Then
             ...
             'Sentencia SQL
             sSQL = "INSERT INTO RPTCBVOUCHERVENTAS VALUES " _
        ...
        ElseIf grdConsulta.TextMatrix(i, 0) = "A" Then
            ...
            'Sentencia SQL
            sSQL = "INSERT INTO RPTCBASIENTOVOUCHERVENTAS VALUES " _
...
Do While Not mcurAsientosVentas.EOF And blnCambioOrden = False
    If mcurAsientosVentas.campo(0) = strOrden Then
        'verificas si es al debe
        If mcurAsientosVentas.campo(3) = "D" Then
                                            
            grdConsulta.AddItem "A" & vbTab & vbTab _
                    & mcurAsientosVentas.campo(1) & vbTab _
                    & vbTab _
                    & mcurAsientosVentas.campo(2) & vbTab _
                    & Format(mcurAsientosVentas.campo(4), "###,###,##0.00") & vbTab _
                    & vbTab & vbTab & vbTab _
                    & vbTab & vbTab & strOrden
                    
    
        ElseIf mcurAsientosVentas.campo(3) = "H" Then
        ...
        mcurAsientosVentas.MoverSiguiente
    Else
...
Set mcolVentasProdServ = Nothing
End Sub
...
'Carga los asientos de Ctb_AsientosVentas, CTB_Asientos y CTB_Asientos_Det
CargaAsientosVentas
...
Private Sub CargaAsientosVentas()
'------------------------------------------
'Propósito  : Carga los asientos de CTB_AsientosVentas
...
sSQL = "SELECT C.Orden, AD.CodContable, PC.DescCuenta, AD.DebeHaber, SUM(AD.Monto) " _
       & "FROM CTB_ASIENTOSVENTAS C, CTB_ASIENTOS A, CTB_ASIENTOS_DET AD, PLAN_CONTABLE PC " _
...
'Copia la sentencia SQL
mcurAsientosVentas.SQL = sSQL

'Verifica si hay error en la sentencia
If mcurAsientosVentas.Abrir = HAY_ERROR Then
    'Termina la ejecución
    End
End If
...
mcurVentas.Cerrar
'curServicioVentas.Cerrar

'Cierra el cursor de asientos contables
mcurAsientosVentas.Cerrar
...
'Accede al registro de la colección
strProdServ = Var30(mcolVentasProdServ.Item(mcurVentas.campo(0)), 2)

'Verifica si es producto
If strProdServ = "P" Then
    'Producto
    DeterminarGlosaConAfecta = "POR LA VENTA DE PRODUCTOS"
ElseIf strProdServ = "S" Then
    'Servicio
    DeterminarGlosaConAfecta = "POR EL COMPROMISO DE LOS SERVICIOS PRESTADOS"
End If
...
'----------------------------------------------
Dim sSQL As String
Dim curServicioVentas As New clsBD2

'Sentencia SQL con cuyos datos se carga el grid
sSQL = ""
sSQL = "SELECT DISTINCT VD.Orden, VD.Concepto " _
       & "FROM VENTAS_DET VD, VENTAS V " _
       & "WHERE V.Orden=VD.Orden And " _
       & "V.FecMov BETWEEN '" & FechaAMD(mskFechaIni) & "' And '" & FechaAMD(mskFechaFin) & "' " _
       & "ORDER BY VD.Orden"
     
'Carga los datos de proveedores
curServicioVentas.SQL = sSQL

'Verifica si hay error
If curServicioVentas.Abrir = HAY_ERROR Then
   'Termina la ejecución
   End
End If

'Hacer mientras no sea fin del registro del cursor
Do While Not curServicioVentas.EOF

    'Agrega los datos a la colección IdCta, DescCta, DescBanco
    mcolVentasProdServ.Add Item:=curServicioVentas.campo(0) & "¯" & _
                                 curServicioVentas.campo(1), _
                            Key:=curServicioVentas.campo(0)
                           
    'Mueve al siguiente registro
    curServicioVentas.MoverSiguiente
Loop
...
'Sentencia SQL
sSQL = "SELECT V.Orden, V.FecMov, V.NumDoc, TD.Abreviatura, V.MontoTotal, V.MontoSubTotal, " _
        & "V.Observ, V.Anulado, VC.DescProveedor " _
        & "FROM VENTAS V, VENTAS_TIPO_DOCUM TD, VENTAS_CLIENTES VC " _
        & "WHERE TD.IdTipoDoc=V.IdTipoDoc And V.IdCliente = VC.IdProveedor And " _
        & "V.FecMov BETWEEN '" & FechaAMD(mskFechaIni) & "' And '" & FechaAMD(mskFechaFin) & "' " _
        & "ORDER BY V.Orden"
...
'Verifica si hay error en la sentencia
If mcurVentas.Abrir = HAY_ERROR Then
    'Termina la ejecución
    End
End If
...
Private Sub CargarVentasEnGrid()
  ...
  'Agrega datos al grdConsulta
  grdConsulta.AddItem "M" & vbTab & FechaDMA(mcurVentas.campo(1)) & vbTab _
                      & mcurVentas.campo(0) & vbTab _
                      & mcurVentas.campo(3) & "/" & mcurVentas.campo(2) & vbTab _
                      & mcurVentas.campo(8) & vbTab _
                      & vbTab & vbTab & vbTab _
                      & Var39(mcurVentas.campo(4)) & vbTab _
                      & vbTab & DeterminarGlosaConAfecta & vbTab & vbTab & mcurVentas.campo(6) _
                      & vbTab & mcurVentas.campo(7)
  ...
  'Carga los Asientos contables para el orden
  CargarAsientos mcurVentas.campo(0)
End Sub
